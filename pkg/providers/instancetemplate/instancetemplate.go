...
func (p *DefaultProvider) Create(ctx context.Context, templateClass *v1alpha1.ProxmoxTemplate) error {
	...
	if len(installedZones) > 0 {
		...
		templateClass.Status.InstalledZones = installedZones
	}
...
func (p *DefaultProvider) Delete(ctx context.Context, templateClass *v1alpha1.ProxmoxTemplate) error {
	...
	for _, key := range templateClass.Status.InstalledZones {
	...
	templateClass.Status.InstalledZones = lo.Filter(templateClass.Status.InstalledZones, func(item string, index int) bool {
		return !slices.Contains(removedImages, item)
	})
	if len(templateClass.Status.InstalledZones) > 0 {
		return fmt.Errorf("unable to delete image %s, still installed in zones: %v", imageID, templateClass.Status.InstalledZones)
	}
...
func (p *DefaultProvider) List(ctx context.Context, nodeClass *v1alpha1.ProxmoxNodeClass) ([]InstanceTemplateInfo, error) {
	...
	for _, region := range regions {
		for _, info := range p.instanceTemplate[region] {
			if info.Status == InstanceTemplateStatusAvailable && info.Name == nodeClass.Spec.InstanceTemplateRef.Name {
