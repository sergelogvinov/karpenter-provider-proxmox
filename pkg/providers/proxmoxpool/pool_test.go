...
func TestNewClient(t *testing.T) {
	...
	pClient, err := pxpool.NewProxmoxPool(t.Context(), []*pxpool.ProxmoxCluster{}, nil)
	assert.NotNil(t, err)
	assert.Nil(t, pClient)

	pClient, err = pxpool.NewProxmoxPool(t.Context(), cfg, nil)
	assert.Nil(t, err)
...
func TestFindVMByNameNonExist(t *testing.T) {
	...
	pClient, err := pxpool.NewProxmoxPool(t.Context(), cfg, &http.Client{})
	assert.Nil(t, err)
	assert.NotNil(t, pClient)
