...
func (topo *CPUTopology) String() string {
	return fmt.Sprintf("CPUs: %d, Cores: %d, Sockets: %d, NUMANodes: %d, UncoreCaches: %d", topo.NumCPUs, topo.NumCores, topo.NumSockets, topo.NumNUMANodes, topo.NumUncoreCache)
}
...
// CPUSocketID returns the socket ID which the given logical CPU belongs to.
...
// CPUNUMANodeID returns the NUMA node ID which the given logical CPU belongs to.
